...
global uchar *copy_buffer_drift_exact,
 global uchar *copy_buffer_cavity,
 global uchar *copy_buffer_align,
    global uchar *copy_buffer_particles, // uint8_t is uchar
    ulong NUM_PARTICLES
    )
{
  size_t gid = get_global_id(0);
  NS(Blocks) copied_beam_elements_drift;
  NS(Blocks) copied_beam_elements_drift_exact;
  NS(Blocks) copied_beam_elements_cavity;
  NS(Blocks) copied_beam_elements_align;
  NS(Blocks_preset)( &copied_beam_elements_drift ); // very important for initialization
  NS(Blocks_preset)( &copied_beam_elements_drift_exact ); // very important for initialization
  NS(Blocks_preset)( &copied_beam_elements_cavity ); // very important for initialization
  NS(Blocks_preset)( &copied_beam_elements_align ); // very important for initialization
  int ret = NS(Blocks_unserialize)(&copied_beam_elements_drift, copy_buffer_drift);
   ret |= NS(Blocks_unserialize)(&copied_beam_elements_drift_exact, copy_buffer_drift_exact);
  ret |= NS(Blocks_unserialize)(&copied_beam_elements_cavity, copy_buffer_cavity);
  ret |= NS(Blocks_unserialize)(&copied_beam_elements_align, copy_buffer_align);
